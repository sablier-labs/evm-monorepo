...
- **Breaking**: Change return type of `totalForgoneAmount()` in `SablierMerkleVCA` from `uint256` to `uint128`
  ([#1363](https://github.com/sablier-labs/evm-monorepo/pull/1363))
- Refactor `SablierMerkleBase` and `SablierMerkleLockup` constructors to accept `ConstructorParams` struct
  ([#1403](https://github.com/sablier-labs/evm-monorepo/pull/1403))
- Refactor `DataTypes` into separate type files ([#1408](https://github.com/sablier-labs/evm-monorepo/pull/1408))
...
- **Breaking**: Add `ClaimType` enum to all campaign deployment parameters
  ([#1405](https://github.com/sablier-labs/evm-monorepo/pull/1405))
- **Breaking**: Add `granularity` parameter to `SablierMerkleLL` deployment parameters for configurable unlock step
  sizes in Linear streams ([#1366](https://github.com/sablier-labs/evm-monorepo/pull/1366))
- **Breaking**: Add `enableRedistribution` boolean parameter to `MerkleVCA.ConstructorParams` struct enabling
  redistribution of forgone tokens ([#1363](https://github.com/sablier-labs/evm-monorepo/pull/1363))
- Add `SablierMerkleExecute` campaign contract in which a function is called on a target contract at claim time
  ([#1393](https://github.com/sablier-labs/evm-monorepo/pull/1393))
- Add `claimViaAttestation` function for attestation-based claiming via EIP-712 signatures from a trusted attestor
  ([#1403](https://github.com/sablier-labs/evm-monorepo/pull/1403))
- Add `sponsor` function to Merkle campaigns ([#1443](https://github.com/sablier-labs/evm-monorepo/pull/1443))

### Removed

- **Breaking**: Remove `aggregateAmount` parameter from `createMerkleVCA` function (moved to
  `MerkleVCA.ConstructorParams` struct parameters) ([#1363](https://github.com/sablier-labs/evm-monorepo/pull/1363))
...
- Replace `createWithDurations` with `createWithTimestamps` in both `MerkleLL` and `MerkleLT` claims
  ([#1024](https://github.com/sablier-labs/evm-monorepo/pull/1024),
  [#28](https://github.com/sablier-labs/airdrops/pull/28))

### Added

- Introduce `SablierMerkleInstant` contract to support campaigns for instantly unlocked airdrops
  ([#999](https://github.com/sablier-labs/evm-monorepo/pull/999))
- Add an option to configure claim fees in the native tokens, managed by the protocol admin. The fee can only be charged
  ...
  ([#1038](https://github.com/sablier-labs/evm-monorepo/pull/1038),
  [#1040](https://github.com/sablier-labs/evm-monorepo/issues/1040))

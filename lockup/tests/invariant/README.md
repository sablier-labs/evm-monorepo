...
9.  State transitions:
    ...
    Note: For a price-gated stream, it is possible for a SETTLED stream to return to STREAMING if the price goes back
    below the target price when end time is not reached.

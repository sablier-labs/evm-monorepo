...
interface ISablierBob is IBatch, IComptrollerable, ISablierBobState {
    ...
    /// @notice Creates a new ERC-20 vault with the specified parameters.
    ///
    ...
    /// - `token` must not be the `NATIVE_TOKEN_PLACEHOLDER`.
    ...
    /// @notice Creates a new native token vault with the specified parameters.
    ///
    /// @dev Emits a {CreateVault} event.
    ///
    /// Notes:
    /// - See noted for {createVault}.
    ///
    /// Requirements:
    /// - `tokenSymbol` must not be empty.
    /// - See {createVault} for other requirements.
    ///
    /// @param oracle The address of the price feed oracle for the native token.
    /// @param expiry The Unix timestamp when the vault expires.
    /// @param targetPrice The target price at which the vault settles, denominated in Chainlink's 8-decimal format for
    /// USD prices, where 1e8 is $1.
    /// @param tokenSymbol The symbol of the native token.
    /// @param tokenDecimals The number of decimals of the native token.
    /// @return vaultId The ID of the newly created vault.
    ...
    /// - For native token vaults, `msg.value` is used as the deposit amount and `amount` is ignored.
    ///
    /// Requirements:
    /// - The vault must have ACTIVE status.
    /// - For ERC-20 vaults, `amount` must be greater than zero and caller must have approved this contract.
    /// - For native token vaults, `msg.value` must be greater than zero.
    ///
    /// @param vaultId The ID of the vault to deposit into.
    /// @param amount The amount of tokens to deposit. Ignored for native token vaults.
    function enter(uint256 vaultId, uint128 amount) external payable;

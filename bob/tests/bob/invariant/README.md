...
2. For a given token $`\tau`$,
   ...
   - Token balance of Bob = $`\sum`$ deposit amount across vaults without adapter + $`\sum`$ tokens received from
     adapter - $`\sum`$ withdrawn amount by users across all vaults.
...
2. For a given vault with adapter,
   ...
   - If `isStakedInAdapter` = false, `wstETH` balance of vault $`\ge`$ $`\sum_{\text{user}}`$ `wstETH` balance of each
     user.

3. If share balance of a user = 0, $`\implies`$ `getYieldBearingTokenBalanceFor` = 0. If the user has share balance > 0,
   `getYieldBearingTokenBalanceFor` > 0.
